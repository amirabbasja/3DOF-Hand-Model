function [odeParams, varargout] = ode15s_Modified(ode,tspan,y0,options,odeParams,varargin)
...
global accepted savedData hasSMASpring;
global spring1 spring1_temp spring1_pure spring2 spring2_temp spring2_pure;
...
while ~done
  ...
  while true                            % Evaluate the formula.
    ...
    if err > rtol                       % Failed step
      ...
      accepted(end+1,:) = [tnew;ynew]'; %#ok<AGROW> 
      accepted_local(end+1,:) = [tnew;ynew]'; %#ok<AGROW> 

      if hasSMASpring
        spring1_temp = spring1;
        spring2_temp = spring2;
        savedData(end+1,1:2) = [spring1.stress spring2.stress]; %#ok<AGROW>
      end
