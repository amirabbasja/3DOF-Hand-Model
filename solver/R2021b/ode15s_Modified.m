function [odeParams, varargout] = ode15s_Modified(ode,tspan,y0,options,dynamicSysParams,odeParams,varargin)
...
accepted_local = [tspan(1) y0'];
savedData = [0 0];
% disp("* Important. The ode15s function is modified by me to support analysis of SMA spring parameters (As global variables). Before using this, change the code. The original code is in the same folder as ode15s and is named 'ode15s - MAIN'. Copy its contents to the ode15s.m file. Note that the ode15s file shouldn't be replaced, only its contents have to be replaced.")
% Abbas: Event function only runs when we have a successfull step.
% Abbas: The code gets the answers on some points (Dont know the logic) and
% after that interpolates between two points to get the rest of the
% answers. Note that interpolation happens after the new point is found (tnew and ynew)
...
while ~done
    % Abbas: The main loop that iterates different steps
  hmin = 16*eps(t);
  ...
  while true                            % Evaluate the formula.
    ...
    while ~gotynew
      ...
      tooslow = false;
      for iter = 1:maxit % Abbas: This iterates the function more than once.
        if Mtype >= 3 
          Mtnew = feval(Mfun,tnew,ynew,Margs{:}); % state-dependent
        end
            % Abbas: The function only runs here
            % Abbas: We require the odeFcn to return dydt and params, so we
            % need to calculate it seperately, get two variables and
            % continue the multiplication with first and update the params
            % with the second one. so i changed this line a little
            odeArgs{:} = dynamicSysParams; % Abbas: added this line so odeArgs would update in each iteration, if we dont do this, odeargs will have the value at the start
            [calc, dynamicSysParams] = feval(odeFcn,tnew,ynew,odeArgs{:});

        rhs = hinvGak*calc -  Mtnew*(psi+difkp1);
          ...
          if ~Jcurrent  
            if Janalytic
              dfdy = feval(Jac,t,y,Jargs{:});
                ...
                % Abbas: This evaluates the function more than once
                odeArgs{:} = dynamicSysParams; % Abbas: added this line so odeArgs would update in each iteration, if we dont do this, odeargs will have the value at the start
                [f0, dynamicSysParams] = feval(odeFcn,t,y,odeArgs{:}); % Abbas: Might run into errrorrrrrr
              [dfdy,Joptions.fac,nF] = odenumjac(odeFcn, {t,y,odeArgs{:}}, f0, Joptions); %#ok<CCAT>
    ...
    % Abbas: if this is true, we have a new y. but it may not meet the requirements 
    ...
    if err > rtol                       % Failed step
        % Abbas: Strat of the failed step
      nfailed = nfailed + 1;            
      if absh <= hmin
        warning(message('MATLAB:ode15s:IntegrationTolNotMet', sprintf( '%e', t ), sprintf( '%e', hmin )));
        solver_output = odefinalize(solver_name, sol,...
      ...
      havrate = false;
        ...
        % Abbas: successfull step happened. update the global
      accepted_local(end+1,:) = [tnew;ynew]'; %#ok<AGROW> 
      if hasSMASpring
          savedData(end+1,1:2) = [dynamicSysParams.spring1.force dynamicSysParams.spring2.force]; %#ok<AGROW>
          dynamicSysParams.spring1_temp = dynamicSysParams.spring1;
          dynamicSysParams.spring2_temp = dynamicSysParams.spring2;
      end
  ...
  if nonNegative && any(ynew(idxNonNegative) < 0)
    ...
    % Abbas: written by me, we stop the simulation loop with this line and
    % we dont use events, because eventFunction doesn't allow us to access
    % the Ys other than current step.
    [~,stop,odeParams] = stopSolverEvent(accepted_local,odeParams.stopParams, odeParams);
    if stop
        % Abbas: if stop is true, break the loop
        done = true;
    end
  ...
  if output_ty || haveOutputFcn 
    switch outputAt
     ...
     case 'RequestedPoints'    % output only at tspan points
      ...
      % Abbas: Interpolating the answers !!!! this may cause trouble
      while next <= ntspan 
...
solver_output = odefinalize(solver_name, sol,...
                            ...
                            {kvec,dif3d,idxNonNegative});
if nargout > 0
    % Abbas: Adding the savedData and accepted_local variables to odeParams
    % so at the end of the simulation we have access to them
    odeParams.savedData = savedData;
    odeParams.accepted_local = accepted_local;
  varargout = solver_output;
end
