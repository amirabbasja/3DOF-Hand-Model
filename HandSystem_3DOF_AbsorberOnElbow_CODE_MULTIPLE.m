function [frequencyRange, amplitude, savedData] = HandSystem_3DOF_AbsorberOnElbow_CODE_MULTIPLE(verbose, plotFRF, saveData, sysObj, options)
    ...
    savedData = struct(); % The variable that contains the SMA data. (Null if no spring used)
    ...
    if system.freqElbow == -1 || system.freqWrist == -1
        freqSweepe = system.freqSweepe;
    else
        freqSweepe = system.freqWrist;
    end
    ...
    for iter = 1:1:length(freqSweepe)
        ...
        % else, the freqsweepe value is a single float number which we will
        % use it as frequency of wrist and elbow
        if system.freqWrist == -1; dynamicSysParams.freqWrist = freqSweepe(iter); else; dynamicSysParams.freqWrist = freqSweepe; end
        if system.freqElbow == -1; dynamicSysParams.freqElbow = freqSweepe(iter); else; dynamicSysParams.freqElbow = freqSweepe; end
